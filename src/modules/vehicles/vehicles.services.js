...
const createVehicle = async (payload) => {
    ...
    const existingVehicle = await db_1.pool.query("SELECT * FROM vehicles WHERE registration_number = $1", [registration_number]);
    ...
    RETURNING id, vehicle_name, type, registration_number, 
              daily_rent_price, availability_status`, [vehicle_name, type, registration_number, daily_rent_price, availability_status]);
    return result.rows[0];
};
const getAllVehicles = async () => {
    const result = await db_1.pool.query(`SELECT id, vehicle_name, type, registration_number, 
            daily_rent_price, availability_status 
     FROM vehicles 
     ORDER BY id`);
    return result.rows;
...
const updateVehicle = async (vehicleId, payload) => {
    ...
    if (registration_number) {
        ...
        const regCheck = await db_1.pool.query("SELECT id FROM vehicles WHERE registration_number = $1 AND id != $2", [registration_number, vehicleId]);
...
const deleteVehicle = async (vehicleId) => {
    ...
    const result = await db_1.pool.query(`DELETE FROM vehicles WHERE id = $1 RETURNING id`, [vehicleId]);
    if (result.rows.length === 0) {
        throw new Error("Vehicle not found");
    }
    return true;
